...
func ValidWindowState(state string) bool {
	return state == WindowPlanned || state == WindowActive || state == WindowCompleted || state == WindowFailed || state == WindowCancelled
}
...
func IsTerminalMission(state string) bool { return state == "aborted" }
